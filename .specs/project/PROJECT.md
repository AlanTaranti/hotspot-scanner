...
**Vision:** Standalone local CLI that identifies maintenance hotspots in TypeScript/JavaScript repositories by combining cyclomatic complexity and Git churn.

**For:** Developers and tech leads who need to prioritize refactoring targets without commercial tooling.

**Solves:** Which files (or functions) are hardest to maintain — complex code that changes often.
...
**Shipped (v1 + post-v1 through M56):**

- `hotspot-scanner` multi-command CLI: `init`, `doctor`, `scan`, `baseline save`, `compare`, `completion`
- `scan` with `--since`, `--format`, `--top`, `--baseline`, `--output`, `--granularity`, `--include` / `--exclude`, `--include-tests`, `--config`, `--concurrency`, `--sequential` / `--no-overlap`, `--only`, `--explain`, `--strict`, `--quiet` / `--no-progress` / `--verbose`, `--dry-run`
- Git Change Miner: streaming numstat pass for file churn; function mode adds sequential pathspec-batched patch streams for per-function hunk-overlap churn
- Complexity Analyzer (McCabe over ts-morph AST; persistent worker-thread pool; `.ts`/`.tsx`/`.js`/`.jsx`/`.mjs`/`.cjs`)
- Hotspot Scorer (file + function granularity)
- CLI table, JSON (`version: "2.0"`), markdown, and CSV bundle output; interpretation UX (summary, glossary, triage); compare deltas for hotspots/functions
...
- Milestone checklist: [ROADMAP.md](ROADMAP.md) (M1–M56 Done)
...
- Temporal coupling analysis (removed M56 — historical specs remain Done for audit)
