# CONCERNS — @taranti/hotspot-scanner
...
| Compiled CLI untested after build          | `tests/compiled-cli.smoke.test.ts` spawns `node dist/bin/hotspot-scanner.js`; skips when `dist/` is missing; gate/CI always `build` before `test` via `pnpm verify` |
