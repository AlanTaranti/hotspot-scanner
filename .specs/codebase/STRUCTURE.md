...
│   ├── scoring/                 # HotspotScorer + TemporalCouplingScorer + static enrich
...
| `src/report/`            | implemented | Reporter — `only`, `summary`, `glossary`, `triage`, `compare-triage`, `explain`, `explain-compare`, `color` (M41/M42/M53); `slice`, `json`, `table`, `markdown`, `coupling-format`, `csv-utils`, `csv-bundle`, `csv`, `slice-compare`, `compare-*` + `createReporter()` factory (`render` + `renderCompare`; `ReporterOptions`: `only`, `triageHints`, `color`; CSV returns `CsvBundle`) |
