...
| `trend <file>` | `src/trend/` (`runComplexityTrend`) via `bin/trend-actions.ts` | Per-file Git history (`listFileRevisions` / `showFileAtRevision` with `--follow` default); indentation metrics + NCLOC per revision; uniform `--max-revisions` sample; ASCII sparklines; `table` \| `json` \| `csv`; separate JSON contract `version: "1.0"` / `kind: "complexity-trend"`; **does not** load config or invoke scan pipeline |
