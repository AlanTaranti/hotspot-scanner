...
describe("createCliDiagnosticHandlers warningsMode", () => {
  ...
  it("emitWarningTeaser writes rollup under summary when buffer non-empty", () => {
    const write = vi
      .spyOn(process.stderr, "write")
      .mockImplementation(() => true);
    const { onWarning, emitWarningTeaser } = createCliDiagnosticHandlers({
      warningsMode: "summary",
    });

    onWarning(createScanWarning("READ_FAILED", "Could not read a.ts"));
    onWarning(createScanWarning("READ_FAILED", "Could not read b.ts"));

    emitWarningTeaser();

    expect(write).toHaveBeenCalledWith(
      "Warnings: 2 total (READ_FAILED: 2)\n",
    );
  });

  it("emitWarningTeaser is no-op when summary buffer empty", () => {
    const write = vi
      .spyOn(process.stderr, "write")
      .mockImplementation(() => true);
    const { emitWarningTeaser } = createCliDiagnosticHandlers({
      warningsMode: "summary",
    });
  ...
  it("emitWarningTeaser is no-op under full and json", () => {
    const write = vi
      .spyOn(process.stderr, "write")
      .mockImplementation(() => true);
    const fullHandlers = createCliDiagnosticHandlers({ warningsMode: "full" });
    const jsonHandlers = createCliDiagnosticHandlers({ warningsMode: "json" });

    fullHandlers.onWarning(createScanWarning("WARN", "msg"));
    jsonHandlers.onWarning(createScanWarning("WARN", "msg"));

    write.mockClear();
    fullHandlers.emitWarningTeaser();
    jsonHandlers.emitWarningTeaser();
