...
| High | `preToolUse` | `pre-edit-guard.mjs` | Planner boundary; fragile `ask`; orchestrated ownership; ARCHITECTURE/CONCERNS/INTEGRATIONS/STACK/STRUCTURE/TESTING SoT `ask` on `M##`/`HOTSPOT-*`; CONVENTIONS SoT `ask` on `M##` |
| High | `postToolUse` | `post-edit-guard.mjs` | Track edits + fragile / scoring / ARCHITECTURE/CONCERNS/INTEGRATIONS/STACK/STRUCTURE/TESTING/CONVENTIONS SoT alerts |
