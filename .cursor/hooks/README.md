...
| High | `preToolUse` | `pre-edit-guard.mjs` | Planner boundary; fragile `ask`; orchestrated ownership; ARCHITECTURE/CONCERNS SoT `ask` on `M##`/`HOTSPOT-*`; CONVENTIONS SoT `ask` on `M##` |
| High | `postToolUse` | `post-edit-guard.mjs` | Track edits + fragile / scoring / ARCHITECTURE/CONCERNS/CONVENTIONS SoT alerts |
