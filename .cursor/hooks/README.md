...
| High | `preToolUse` | `pre-edit-guard.mjs` | Planner boundary; fragile `ask`; orchestrated ownership; ARCHITECTURE/CONCERNS/INTEGRATIONS/STACK/STRUCTURE/TESTING/PROJECT SoT `ask` on `M##`/`HOTSPOT-*`; CONVENTIONS/AGENTS SoT `ask` on `M##`; CONTRIBUTING SoT `ask` on SoT-mirror dumps (`M##`, directory trees, Coverage thresholds, exit-code tables, Architecture boundaries / Fragile areas H2); README SoT `ask` on adoption drift (`## Advanced` / Features / Pipeline detail / Performance / Rename / Command synopsis / `M##`); ROADMAP SoT `ask` on drift patterns (Artifacts/HOTSPOT/tasks dump; `M##` allowed); STATE SoT `ask` on execute-log drift (`Execute complete` / `Specs Planned` / etc.; `M##` allowed in locks) |
